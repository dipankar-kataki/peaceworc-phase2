...
namespace App\Http\Controllers\Caregiver\Notification;
...
class CaregiverNotificationController extends Controller
{
    use ApiResponse;
    ...
    public function markNotificationAsRead(Request $request){
        ...
        if($validator->fails()){
            ...
            try{
                $check_if_notification_exists = CaregiverNotification::where('id', $request->notification_id)->where('user_id', Auth::user()->id)->exists();

                if(!$check_if_notification_exists){
                    return $this->error('Oops! Failed to mark as read. Notification does not exists.', null, null, 400);
                }else{
                    CaregiverNotification::where('id', $request->notification_id)->where('user_id', Auth::user()->id)->update([
                        'mark_as_read' => 1
                    ]);
    ...
    public function getUnreadNotificationCount(){
        try{
            $get_unread_notification_count = CaregiverNotification::where('user_id', Auth::user()->id)->where('mark_as_read', 0)->count();
