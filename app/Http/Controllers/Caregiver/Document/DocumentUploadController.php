...
class DocumentUploadController extends Controller
{
    ...
    public function updateStatus(Request $request){
        try{
            $covid_count = CovidDocument::where('user_id', Auth::user()->id)->where('status', 1)->count();
            $child_count = ChildAbuseDocument::where('user_id', Auth::user()->id)->where('status', 1)->count();
            $criminal_count = CriminalDocument::where('user_id', Auth::user()->id)->where('status', 1)->count();
            $driving_count = DrivingDocument::where('user_id', Auth::user()->id)->where('status', 1)->count();
