...
class JobOngoingController extends Controller
{
    use ApiResponse;
    public function getOngoingJob(){
        try{
            $get_upcoming_job = AgencyPostJob::where('user_id', Auth::user()->id)->where('status', JobStatus::OnGoing)->latest()->get();
            $all_details = [];
            foreach($get_upcoming_job as $job){
                $check_accepted_by = AcceptJob::where('job_id', $job->id)->where('status', JobStatus::OnGoing)->latest()->get();
                ...
                foreach($check_accepted_by as $accepted_by){
                    ...
                    $details = [
                        ...
                        'end_date' => Carbon::parse($job->end_date)->format('m-d-Y'),
            ...
            return $this->success('Great! Ongoing Jobs Fetched Successfully.',  $all_details, null, 200);
        }catch(\Exception $e){
